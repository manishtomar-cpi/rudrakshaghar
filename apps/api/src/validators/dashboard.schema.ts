...
export const DashboardRangeEnum = z.enum(["today", "7d", "30d", "90d", "custom"]);
...
export const DashboardQuerySchema = z
  .object({
    ...
    // keep existing bounds: default 5, min 1, max 20
    limit: z.preprocess(
  ...
  .superRefine((val, ctx) => {
    if (val.range === "custom") {
      if (!val.from)
        ctx.addIssue({
          code: "custom",
          message: "`from` required for custom range",
        });
      if (!val.to)
        ctx.addIssue({
          code: "custom",
          message: "`to` required for custom range",
        });
    }
  });
